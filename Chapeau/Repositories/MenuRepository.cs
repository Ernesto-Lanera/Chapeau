    ...
    public class MenuRepository(IConfiguration configuration, ILogger<MenuRepository> logger)
    {
        ...
        public List<MenuItem> GetMenuItems(int? cardId, int? categoryId)
        {
            ...
            try
            {
                ...
                if (reader.HasRows)
                    menuItems = MapMenuItemsFromReader(reader);
            }
            catch (SqlException ex)
            {
                _logger.LogError(ex, "SQL error retrieving menu items");
        ...
        public void AddMenuItem(MenuItem item)
        {
            try
            {
                ...
                string query = @"INSERT INTO MenuItems (Name, Price, Stock, IsActive, CategoryID) 
                               VALUES (@Name, @Price, @Stock, @IsActive, @CategoryID)";
        ...
        public void UpdateMenuItem(MenuItem item)
        {
            try
            {
                ...
                string query = @"UPDATE MenuItems SET Name = @Name, Price = @Price, Stock = @Stock, 
                               IsActive = @IsActive, CategoryID = @CategoryID WHERE MenuItemID = @MenuItemID";
